...
| `timeSignature` | Text (Short text) | **Max length:** 10, **Required:** false, **Placeholder:** "e.g., 4/4, 3/4, 6/8, 7/8", **Regex:** `^[1-9]\d{0,1}/(?:2|4|8|16|32)$` |
| `musicalKey` | Enumeration | **Values:** `C`, `C#`, `D`, `D#`, `E`, `F`, `F#`, `G`, `G#`, `A`, `A#`, `B` |
...
- `musicalKey` is the root note (C, G, A, etc.) - allows filtering by key
...
- `isInstrumental` enables filtering vocal vs instrumental tracks
...
const trackInfo = {
  ...
  type: track.isInstrumental ? "Instrumental" : "Vocal",

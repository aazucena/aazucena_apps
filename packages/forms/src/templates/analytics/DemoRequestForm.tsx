...
import { demoRequestSchema, type DemoRequestFormData } from '../../schemas/index';
...
export function DemoRequestForm({
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      currentTools: '',
      ...defaultValues,
    } as DemoRequestFormData,
    onSubmit: async ({ value }: { value: any }) => {
      try {
        onSuccess?.(demoRequestSchema.parse(value));
      } catch (error) {
        onError?.(error);
      }
    },
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3">
        <ControlledInput
          ...
          required
          validators={{ onChange: demoRequestSchema.shape.name }}
        />
        <ControlledInput
          name="email"
          label="Email"
          ...
          required
          validators={{ onChange: demoRequestSchema.shape.email }}
        />
      </div>
      ...
      <ControlledTextarea
        name="useCase"
        label="Use Case"
        placeholder="What will you use AZUCENA_LYTICS for?"
        required
        validators={{ onChange: demoRequestSchema.shape.useCase }}
      />
      <ControlledInput
        name="currentTools"
        label="Current Tools"
        placeholder="What analytics tools do you use now?"
      />

...
import { musicFeedbackSchema, type MusicFeedbackFormData } from '../../schemas/index';
...
export function MusicFeedbackForm({
  variant = 'default',
  onSuccess,
  onError,
  className,
  defaultValues,
  trackTitle,
  feedbackElements = DEFAULT_ELEMENTS,
}: MusicFeedbackFormProps) {
  const form = useForm({
    validatorAdapter: zodValidator(),
    defaultValues: {
      formType: FormTypeEnum.Values['Music Feedback'],
      name: '',
      email: '',
      subject: trackTitle ? `Feedback: ${trackTitle}` : '',
      message: '',
      trackTitle: trackTitle ?? '',
      elements: [] as string[],
      ...defaultValues,
    } as MusicFeedbackFormData,
    onSubmit: async ({ value }: { value: any }) => {
      try {
        onSuccess?.(musicFeedbackSchema.parse(value));
      } catch (error) {
        onError?.(error);
      }
    },
  ...
  return (
    ...
    >
      ...
      <ControlledInput
        name="trackTitle"
        label="Track Title"
        placeholder="Which track are you reviewing?"
      />
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      field.handleChange(isSelected ? cur.filter((e) => e !== el) : [...cur, el]);
                    }}
                    className={cn(

...
export function TutorialRequestForm({ variant = 'default', onSuccess, onError, className, defaultValues }: TutorialRequestFormProps) {
  const form = useForm({
    ...
    onSubmit: async ({ value }: { value: any }) => {
      try { onSuccess?.(tutorialRequestSchema.parse(value)); } catch (error) { onError?.(error); }
    },
  } as any);
  ...
  return (
    ...
    >
      <FormErrorSummary />
      <ControlledInput name="topic" label="Tutorial Topic" placeholder="Building a real-time dashboard with Next.js and ClickHouse" required validators={{ onChange: tutorialRequestSchema.shape.topic }} />
              ...
              {(['video', 'written', 'interactive', 'all'] as const).map((f) => (
                <button
                  ...
                  className={cn(
                    ...
                    field.state.value === f ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-background hover:border-primary/50'
                  )}
                >
                  {f}
                </button>
                ...
                {(['beginner', 'intermediate', 'advanced'] as const).map((s) => (
                  <button
                    ...
                    className={cn(
                      ...
                      field.state.value === s ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-background hover:border-primary/50'
                    )}
                  >
                    {s}
                  </button>
              ...
              <div className="flex flex-col gap-1">
                {(['no rush', 'within a month', 'this week', 'asap'] as const).map((u) => (
                  <button
                    ...
                    className={cn(
                      'rounded-md border px-3 py-1.5 text-xs font-medium capitalize transition-all text-left',
                      field.state.value === u ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-background hover:border-primary/50'
                    )}
                  >
                    {u}
                  </button>
      ...
      <ControlledTextarea name="additionalContext" label="Additional Context (optional)" placeholder="Why this topic? What problem are you trying to solve?" />

...
import { referralSchema, type ReferralFormData } from '../../schemas/index';
...
export function ReferralForm({
  ...
  const form = useForm({
    ...
    defaultValues: {
      formType: FormTypeEnum.Values.Referral,
      name: '',
      email: '',
    ...
    onSubmit: async ({ value }: { value: any }) => {
      try {
        onSuccess?.(referralSchema.parse(value));
      } catch (error) {
        onError?.(error);
      }
    },
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3">
        <ControlledInput
          ...
          required
          validators={{ onChange: referralSchema.shape.referralName }}
        />
        <ControlledInput
          name="referralEmail"
          ...
          required
          validators={{ onChange: referralSchema.shape.referralEmail }}
        />
      </div>
      <ControlledTextarea
        name="message"

...
import { guestbookSchema, type GuestbookFormData } from '../../schemas/index';
...
export function GuestbookForm({
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }: { value: any }) => {
      try {
        onSuccess?.(guestbookSchema.parse(value));
      } catch (error) {
        onError?.(error);
      }
    },
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3">
        <ControlledInput
          ...
          required
          validators={{ onChange: guestbookSchema.shape.name }}
        />
        <ControlledInput
          name="website"
      ...
      <ControlledTextarea
        ...
        placeholder="Love the work! Inspired by the animations…"
        required
        validators={{ onChange: guestbookSchema.shape.message }}
      />
      ...
      <ControlledCheckbox
        name="publicConsent"
        label="I consent to my message being displayed publicly"
        required
        validators={{ onChange: guestbookSchema.shape.publicConsent }}
      />

...
export function CollaborationForm({ variant = 'default', onSuccess, onError, className, defaultValues }: CollaborationFormProps) {
  const form = useForm({
    validatorAdapter: zodValidator(),
    defaultValues: { formType: FormTypeEnum.Values.Collaboration, name: '', email: '', subject: '', message: '', projectType: '', budget: '', timeline: '', ...defaultValues } as CollaborationFormData,
    onSubmit: async ({ value }: { value: any }) => {
      try { onSuccess?.(collaborationSchema.parse(value)); } catch (error) { onError?.(error); }
    },
  } as any);
  return (
    <Form form={form} variant={variant} className={cn('max-w-lg space-y-4', className)} onSubmit={(e) => { e.preventDefault(); form.handleSubmit(); }}>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <ControlledInput name="email" label="Email" type="email" placeholder="jane@example.com" required />
      </div>
      <ControlledInput name="subject" label="Collaboration Type" placeholder="Speaking, Writing, Partnership…" required />
      <div className="grid grid-cols-2 gap-3">
      ...
      <ControlledTextarea name="message" label="Details" placeholder="Tell me about the opportunity…" required />

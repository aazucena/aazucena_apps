...
export function calculateCareerStats(experiences: Experience[]): CareerStat {
  ...
  let totalYears = 0;
  if (validStartDates.length > 0 && validStartDates[0]) {
...
export function calculateSkillGrowthMetrics(
  ...
  const processSkills = (skills: (string | SkillWithCategory)[], year?: number) => {
    skills.forEach((s) => {
      ...
      if (year) {
        const yearSet = skillsByYear.get(year);
        if (yearSet) {
          yearSet.add(name);
        } else {
          const newSet = new Set<string>();
          newSet.add(name);
          skillsByYear.set(year, newSet);
        }
      }
    });
  };
  ...
  const yearRange =
    years.length > 0 && years[0] !== undefined && years[years.length - 1] !== undefined
      ? (years[years.length - 1] as number) - (years[0] as number) + 1
      : 1;

  ...
  return (
    <div className="absolute top-6 right-6 z-20 w-52 transition-all duration-500 opacity-90 hover:opacity-100 scale-95 hover:scale-100 origin-top-right">
      <Card
        variant="cyber"
        padding="none"
        className="overflow-hidden backdrop-blur-xl border-cyan-500/30 shadow-[0_0_30px_rgba(6,182,212,0.1)]"
      >
        ...
        <CardContent className="p-2.5 flex flex-col gap-1.5">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                <div className="flex items-center">
                  ...
                  <span className="text-[9px] font-mono uppercase tracking-widest ml-2">
                    Show_Grid
                  </span>
                </div>
            ...
            <TooltipContent variant="cyber" side="left">
              Toggles coordinate floor grid
            </TooltipContent>
          </Tooltip>
              ...
              >
                <div className="flex items-center">
                  <Icons.Refresh
                    ...
                    className={
                      autoRotate
                        ? 'animate-spin-slow text-cyan-400'
          ...
          <Tooltip>
            ...
            <TooltipContent variant="cyber" side="left">
              Toggle geometry wireframe mode
            </TooltipContent>
          </Tooltip>
          ...
          <DropdownMenu>
            <Tooltip>
              <TooltipTrigger asChild>
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    <div className="flex items-center">
                      <Icons.Sun size={14} />
                      <span className="text-[9px] font-mono uppercase tracking-widest ml-2 truncate">
                        Env: {envPreset}
                      </span>
                    </div>
              ...
              <TooltipContent variant="cyber" side="left">
                Select HDR lighting environment
              </TooltipContent>
            </Tooltip>
            <DropdownMenuContent variant="cyber" side="left" align="start" className="w-48">
              <DropdownMenuLabel className="text-[9px] font-mono uppercase tracking-widest text-zinc-500 px-2 py-1.5">
                Lighting_Presets
              </DropdownMenuLabel>
              <DropdownMenuSeparator className="bg-cyan-500/10" />
              <DropdownMenuRadioGroup
                value={envPreset}
                onValueChange={(v) => setEnvPreset(v as PresetsType)}
              >
                {ENV_PRESETS.map((preset) => (
                  <DropdownMenuRadioItem
                    key={preset}
                    value={preset}
                    className="text-[10px] font-mono uppercase tracking-widest focus:bg-cyan-500/10 focus:text-cyan-400 py-2"
                  >
                    {preset}
                  </DropdownMenuRadioItem>
                ))}
          ...
          <div className="flex gap-1 mb-4">
            ...
            >
              <Icons.Camera
                size={14}
                className={`transition-transform duration-300 ${isCapturing ? 'scale-75 text-cyan-400' : 'group-hover/snap:scale-110'}`}
              />
            </Button>
            ...
            >
              <Icons.Refresh
                size={14}
                className={`transition-transform duration-700 ${isResetting ? 'rotate-[360deg] text-cyan-400' : 'group-hover/reset:rotate-[-90deg]'}`}
              />
            </Button>
          </div>
        </CardContent>

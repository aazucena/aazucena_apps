  ...
  return (
    <group ref={tardisRef}>
      ...
      <mesh castShadow>
        ...
        <meshStandardMaterial color="#1e3a8a" transparent opacity={opacity} metalness={0.3} roughness={0.7} />
      </mesh>
      ...
      {[0.4, 0.1].map((y) => (
        [-0.2, 0.2].map((x) => (
          <mesh key={`${x}-${y}`} position={[x, y + 0.3, 0.41]}>
            ...
            <meshStandardMaterial color="#ffffff" emissive="#f8fafc" emissiveIntensity={0.5} transparent opacity={opacity * 0.8} />
          </mesh>
        ))
      ))}
    </group>

...
import type { PixiParticle, ParticleRendererConfig } from '@aazucena/types';
export class ParticleRenderer {
   ...
   */
  createGraphics(particles: PixiParticle[]): void {
    this.clearGraphics();
    ...
    particles.forEach((particle) => {
      ...
      sprite.blendMode = this.config.blendMode as any;
   ...
   */
  updateGraphics(particles: PixiParticle[]): void {
    // Apply effect-based alpha multiplier
    const alphaMultiplier =
      this.currentEffect === 'glow' ? 1.5 : this.currentEffect === 'blur' ? 1.2 : 1.0;
   ...
   */
  render(particles: PixiParticle[]): void {

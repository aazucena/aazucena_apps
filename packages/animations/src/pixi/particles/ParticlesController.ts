...
import type { ParticleSystemConfig, PixiParticle } from '@aazucena/types';
import { PARTICLE_DEFAULTS, PARTICLE_PRESETS } from '@aazucena/constants';
...
export class ParticlesController {
   ...
   */
  private createParticle(width: number, height: number): PixiParticle {
  ...
  update(deltaTime: number, width: number, height: number): void {
    ...
    this.particles.forEach((particle) => {
      ...
      const twinkle =
        Math.sin(currentTime * particle.twinkleSpeed + particle.twinklePhase) * 0.5 + 0.5;

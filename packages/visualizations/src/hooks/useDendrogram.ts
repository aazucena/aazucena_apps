...
export interface UseDendrogramOptions {
  width: number;
  height: number;
...
export function useDendrogram<T extends DendrogramNode>(
  svgRef: React.RefObject<SVGSVGElement | null>,
  data: T,
  { width, height, direction, color, onNodeClick }: UseDendrogramOptions,
) {
  useEffect(() => {
    ...
    node
      ...
      .attr('stroke-width', 3);
  }, [svgRef, data, width, height, direction, color, onNodeClick]);
}

...
export async function getSkillCategories(): Promise<SkillCategoryInfo[]> {
  try {
    ...
    if (error instanceof z.ZodError) {
      console.error('[Skill Categories] Invalid CMS data structure:', error.issues);
 ...
 */
export async function getSkillCategoryByName(name: string): Promise<SkillCategoryInfo | undefined> {

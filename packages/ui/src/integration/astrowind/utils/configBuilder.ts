...
const  getApp = (config: Config) => {
  ...
  return merge({}, _default, config?.apps ?? {});
...
const configBuilder =  (config: Config) => ({
  SITE: getSite(config),
  I18N: getI18N(config),
...
export interface ConfigBuilder extends ReturnType<typeof configBuilder> {};
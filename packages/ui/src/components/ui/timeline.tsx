...
const TimelineItem = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn('group relative pb-8 last:pb-0 md:pb-12', className)} {...props} />
  ),
);
...
const timelineDotVariants = cva(
  'flex h-6 w-6 items-center justify-center rounded-full transition-all duration-300 group-hover:scale-110 shadow-lg',
  {
    variants: {
      variant: {
        default: 'bg-gradient-to-br from-white/40 to-white/20',
  ...
  ({ className, variant, ...props }, ref) => {
    return (
      <div className="absolute top-1/2 left-0 z-10 flex -translate-y-1/2 items-center justify-center">
        <div ref={ref} className={cn(timelineDotVariants({ variant }), className)} {...props}>

...
const FileTree = React.forwardRef<HTMLDivElement, FileTreeProps>(
  (
    ...
    const [expandedNodes, setExpandedNodes] = React.useState<Set<string>>(new Set(defaultExpandedIds));
    ...
    const renderNode = (node: FileTreeNode) => {
      ...
      return (
        <li key={node.id} className={cn('flex flex-col', size === 'sm' && 'py-0.5', size === 'lg' && 'py-1')}>
          <div
            className={cn(
          ...
          >
            ...
            {isFolder ? (
              <Folder className={cn('h-4 w-4 shrink-0', isExpanded ? 'text-blue-500' : 'text-muted-foreground')} />
            ) : (
          ...
          {isFolder && isExpanded && node.children && (
            <ul className="ml-4 border-l border-gray-500/20">

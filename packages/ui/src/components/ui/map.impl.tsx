 ...
 */
export function useDivIcon({ html, className = '' }: UseCustomIconProps) {
  const [icon, setIcon] = React.useState<any>(null);

  React.useEffect(() => {
    getDivIcon(html, className).then(setIcon);
  }, [html, className]);
...
export const defaultMarkerIcon = () => {
  ...
  const icon = useDivIcon({ html, className: 'bg-transparent border-none' });
  return icon;
};

  ...
  'group/link transition-all duration-500 active:scale-95 inline-flex items-center gap-2 font-black uppercase tracking-[0.2em] no-underline',
  {
    variants: {
      variant: {
        ...
        cyber:
          'text-primary dark:text-cyan-400 hover:text-primary/80 dark:hover:text-cyan-300 font-mono italic',
        ...
        'circular-cyber':
          'gap-3 text-xs text-primary dark:text-cyan-400 hover:text-primary/80 dark:hover:text-cyan-300 font-mono italic',
      },
      size: {
        xs: 'text-[9px]',
        sm: 'text-[10px]',
...
const ArrowLink = React.forwardRef<HTMLAnchorElement, ArrowLinkProps>(
  (
    ...
    const iconStyle =
      variant === 'circular-cyber' ? 'circular-cyber' : isCircular ? 'circular' : 'default';

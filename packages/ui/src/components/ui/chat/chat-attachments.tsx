...
export interface ChatAttachmentsProps
  extends
    ...
    VariantProps<typeof chatAttachmentsVariants> {
      ...
      <div ref={ref} className={cn(chatAttachmentsVariants({ variant }), className)} {...props}>
        ...
        <ul className="flex flex-col gap-2">
          {attachments.map(attachment => {
            ...
            return (
              <li key={attachment.id} className="flex items-center gap-3 rounded-md bg-muted/50 p-2 text-sm">
                {attachment.preview ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={attachment.preview} alt="Preview" className="h-8 w-8 rounded-md object-cover" />
                ) : (
                ...
                <div className="flex flex-col flex-grow">
                  ...
                  {attachment.status && (
                    <div className="flex items-center gap-1 text-xs">
                      <span className={statusColor}>{attachment.status.charAt(0).toUpperCase() + attachment.status.slice(1)}</span>
                      {attachment.progress !== undefined && attachment.status === 'uploading' && (
                        <span className="text-muted-foreground">({attachment.progress}%)</span>
                      )}
                      {attachment.error && <span className="text-red-500"> - {attachment.error}</span>}
                    </div>
                  )}
                </div>
                <div className="flex items-center gap-1">
                  {attachment.url && attachment.status === 'uploaded' && (
                    <a href={attachment.url} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground">
                      <Download className="h-4 w-4" />
                    </a>
                  )}
                  {onRemoveAttachment && attachment.status !== 'uploading' && (
                    <button type="button" onClick={() => onRemoveAttachment(attachment.id)} className="text-muted-foreground hover:text-red-500">

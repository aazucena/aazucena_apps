...
import { ChevronDown, ChevronLeft, ChevronRight } from '@aazucena/icons';
...
const calendarVariants = cva('p-4 transition-all duration-300', {
  variants: {
    variant: {
      ...
      glass:
        'bg-background/5 dark:bg-white/5 backdrop-blur-md border border-border/10 text-foreground rounded-xl shadow-xl',
      cyber:
        'bg-background/80 dark:bg-black/80 border border-cyan-500/30 text-foreground shadow-[0_0_15px_rgba(6,182,212,0.1)] rounded-xl',
    },
  },
  defaultVariants: {
    variant: 'default',
...
function Calendar({
  ...
  const navButtonVariant =
    variant === 'glass' ? 'glass' : variant === 'cyber' ? 'cyber' : buttonVariant;
    ...
    <DayPicker
      ...
      classNames={{
        ...
        button_previous: cn(
          ...
          variant !== 'default' && 'border-none shadow-none',
        ...
        weekday: cn(
          'text-muted-foreground flex-1 select-none rounded-md pb-1 text-[0.8rem] font-normal',

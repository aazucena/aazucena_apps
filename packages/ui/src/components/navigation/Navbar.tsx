...
import { ThemeToggle } from '../status/ThemeToggle.js';
...
export function Navbar({ siteName, currentPath, logoUrl, navItems, className }: NavbarProps) {
    ...
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          {/* Logo */}
          <a href="/" className="group relative z-[110] flex items-center gap-3">
            {logoUrl ? (
              <img
                src={logoUrl}
                alt={siteName}
            ...
            {ctaButtons.map((button) => {
              ...
              return (
                <a
                  ...
                  className={cn(
                    'group hidden items-center gap-2 rounded-full px-6 py-2.5 text-sm font-bold shadow-lg transition-all active:scale-95 sm:flex',
                    buttonClasses[button.buttonStyle || 'primary'],
                  )}
                ...
                {regularNavItems.map((item) => {
                  ...
                  return (
                    <a
                      ...
                      href={item.path || undefined}
                      className={cn(
                        'flex items-center gap-4 rounded-2xl p-4 text-base font-bold transition-all',
                        isActive
                ...
                {ctaButtons.map((button) => {
                  ...
                  return (
                    <a
                      ...
                      href={button.path || undefined}
                      className={cn(
                        'flex w-full items-center justify-center gap-3 rounded-[1.5rem] py-5 text-sm font-bold',
                        mobileButtonClasses[button.buttonStyle || 'primary'],
                      )}
                      target={button.type === 'EXTERNAL' ? '_blank' : undefined}
                      rel={button.type === 'EXTERNAL' ? 'noopener noreferrer' : undefined}
                    >

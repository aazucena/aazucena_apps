...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'A linear set of two or more segments, each of which functions as a mutually exclusive button.',
      },
    },
  },
  tags: ['autodocs'],
...
export const Basic: Story = {
  render: () => {
    ...
    return (
      <SegmentedControl>
        <SegmentedItem value="all" isActive={value === "all"} onClick={() => setValue("all")}>All_Nodes</SegmentedItem>
        <SegmentedItem value="active" isActive={value === "active"} onClick={() => setValue("active")}>Active</SegmentedItem>
        <SegmentedItem value="pending" isActive={value === "pending"} onClick={() => setValue("pending")}>Pending</SegmentedItem>
      </SegmentedControl>
    );
  },
};
    ...
    return (
      <div className="p-12 bg-black rounded-[3rem] border border-cyan-500/10">
        <div className="flex flex-col items-center gap-6">
          <p className="text-[10px] font-mono text-cyan-500/40 uppercase tracking-[0.4em]">Signal_Source_Override</p>
          <SegmentedControl {...args}>
            <SegmentedItem variant="cyber" value="live" isActive={value === "live"} onClick={() => setValue("live")}>
              <Activity className="size-3 mr-2" /> LIVE_STREAM
            </SegmentedItem>
            <SegmentedItem variant="cyber" value="archive" isActive={value === "archive"} onClick={() => setValue("archive")}>
    ...
    return (
      <div className="p-20 bg-gradient-to-tr from-indigo-600 via-blue-700 to-cyan-800 rounded-[3rem] flex flex-col items-center gap-8">
        <SegmentedControl {...args} className="bg-white/5 border-white/10 shadow-2xl px-2 py-1.5">
          <SegmentedItem variant="glass" value="v1" isActive={value === "v1"} onClick={() => setValue("v1")} className="px-8 text-white">
            PHASE_01
          </SegmentedItem>
          <SegmentedItem variant="glass" value="v2" isActive={value === "v2"} onClick={() => setValue("v2")} className="px-8 text-white">
            PHASE_02
          </SegmentedItem>
        </SegmentedControl>
  ...
  render: (args) => {
    ...
    return (
      <div className="flex items-center gap-4 border p-2 rounded-2xl bg-muted/20">
        ...
        <SegmentedControl {...args}>
          <SegmentedItem value="grid" isActive={value === "grid"} onClick={() => setValue("grid")}>GRID</SegmentedItem>
          <SegmentedItem value="list" isActive={value === "list"} onClick={() => setValue("list")}>LIST</SegmentedItem>
        </SegmentedControl>
      </div>
    );
  },

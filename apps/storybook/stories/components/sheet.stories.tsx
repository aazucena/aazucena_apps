...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'A content panel that slides in from the edge of the screen. Also known as a side drawer.',
      },
    },
  },
  tags: ['autodocs'],
  ...
  render: () => (
    <Sheet>
      ...
      <SheetContent side="right">
        ...
        <SheetFooter>
          <Button className="w-full h-12 rounded-full uppercase font-black tracking-widest">Commit_Changes</Button>
        </SheetFooter>
      </SheetContent>
    </Sheet>
  ),
...
export const CyberTerminal: Story = {
  render: () => (
    <Sheet>
      <SheetTrigger asChild>
        <Button variant="cyber"><Activity className="mr-2 animate-pulse" /> ACCESS_NODE_CONTROLS</Button>
      </SheetTrigger>
      <SheetContent variant="cyber" side="left" className="font-mono">
        <SheetHeader className="border-b border-cyan-500/10 pb-6 mb-6">
          <div className="flex items-center gap-3 text-cyan-500 mb-2">
            <Terminal size={18} />
            <SheetTitle className="text-cyan-500 font-mono italic uppercase tracking-tighter text-2xl">// CORE_SHELL_V4</SheetTitle>
          </div>
          <SheetDescription className="text-cyan-500/40 text-[10px] uppercase">Enclave_Authorization: LEVEL_01</SheetDescription>
        </SheetHeader>
        ...
        <div className="space-y-8">
          <div className="p-4 bg-cyan-500/5 border border-cyan-500/10 rounded-xl space-y-4">
            <div className="flex justify-between items-center">
              ...
              <Badge variant="cyber" size="xs">NOMINAL</Badge>
            </div>
          ...
          <div className="space-y-4">
            <h4 className="text-[10px] font-black uppercase opacity-20 tracking-[0.3em]">Quick_Actions</h4>
            <div className="flex flex-col gap-2">
              <Button variant="ghost" size="sm" className="justify-start italic hover:text-cyan-400 font-mono">{'>'} FLUSH_BUFFER</Button>
              <Button variant="ghost" size="sm" className="justify-start italic hover:text-cyan-400 font-mono">{'>'} ROTATE_KEYS</Button>
              <Button variant="ghost" size="sm" className="justify-start italic hover:text-rose-400 font-mono">{'>'} EMERGENCY_REBOOT</Button>
            </div>
          </div>
        </div>
      </SheetContent>
...
export const GlassFloating: Story = {
  render: () => (
    <div className="p-40 bg-gradient-to-tr from-indigo-600 via-blue-700 to-cyan-800 rounded-[4rem] relative overflow-hidden flex items-center justify-center">
      ...
      <Sheet>
        <SheetTrigger asChild>
          <Button variant="glass" className="bg-white/10 text-white border-white/20 hover:bg-white/20 rounded-full px-10 h-14 font-black uppercase tracking-widest">
            Inspect_Environment
          </Button>
        </SheetTrigger>
        <SheetContent variant="glass" side="right" layout="floating" className="border-white/10 text-white w-[400px]">
          <SheetHeader>
            <SheetTitle className="text-white text-3xl font-black tracking-tighter">Atmospheric_Unit</SheetTitle>
            <SheetDescription className="text-white/40">Real-time refraction metrics</SheetDescription>
          </SheetHeader>
          <div className="py-12 space-y-8">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-white/10 rounded-2xl border border-white/20"><Globe size={24}/></div>
            ...
            <div className="p-6 bg-white/5 border border-white/10 rounded-2xl">
              <p className="text-xs leading-relaxed text-white/80">Adaptive glass surfacing is performing within optimal parameters. Backdrop blur constant set to **32px**.</p>
            </div>
          </div>
          <SheetFooter className="mt-auto pt-8 border-t border-white/10">
            <Button variant="outline" className="w-full h-12 rounded-full border-white/20 text-white hover:bg-white/10 font-bold uppercase tracking-widest text-[10px]">
              Sync_Atmosphere
            </Button>
          </SheetFooter>
        </SheetContent>

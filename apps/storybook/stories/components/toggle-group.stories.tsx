...
import { Layout, Globe, Activity, TypeBold as Bold, TypeItalic as Italic, TypeUnderline as Underline, Database, Shield, Zap } from '@aazucena/icons';
...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'A set of two-state buttons that can be toggled on or off, functioning as either a single or multiple selection group.',
      },
    },
  },
  tags: ['autodocs'],
...
export const MultiSelect: Story = {
  ...
  render: (args) => (
    <div className="flex flex-col items-center gap-4">
      ...
      <ToggleGroup {...args}>
        <ToggleGroupItem value="grid" aria-label="Toggle grid"><Layout /></ToggleGroupItem>
        <ToggleGroupItem value="map" aria-label="Toggle map"><Globe /></ToggleGroupItem>
        <ToggleGroupItem value="live" aria-label="Toggle live"><Activity /></ToggleGroupItem>
      </ToggleGroup>
    </div>
  ),
};
  ...
  render: (args) => (
    <div className="p-10 bg-black border border-cyan-500/10 rounded-2xl flex flex-col items-center gap-8">
      <div className="flex items-center gap-3">
        ...
        <span className="font-mono text-xs text-cyan-500 uppercase tracking-tighter italic">// SELECT_ACTIVE_SECTOR</span>
      </div>
      <ToggleGroup {...args} className="gap-2">
        <ToggleGroupItem value="s1" className="h-12 w-16 border-cyan-500/30 text-cyan-500 data-[state=on]:bg-cyan-500/20 data-[state=on]:text-cyan-400 data-[state=on]:border-cyan-400">0x7F</ToggleGroupItem>
        <ToggleGroupItem value="s2" className="h-12 w-16 border-cyan-500/30 text-cyan-500 data-[state=on]:bg-cyan-500/20 data-[state=on]:text-cyan-400 data-[state=on]:border-cyan-400">0x1A</ToggleGroupItem>
        <ToggleGroupItem value="s3" className="h-12 w-16 border-cyan-500/30 text-cyan-500 data-[state=on]:bg-cyan-500/20 data-[state=on]:text-cyan-400 data-[state=on]:border-cyan-400">0x9C</ToggleGroupItem>
      </ToggleGroup>
    </div>
  ),
};
...
export const TypographyTools: Story = {
  render: () => (
    <ToggleGroup type="multiple" variant="outline" className="border p-1 rounded-xl bg-card shadow-sm">

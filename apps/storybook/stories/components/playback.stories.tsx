...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          'A playback control bar for navigating sequential data or temporal streams. Optimized for analytical terminals and technical walkthroughs.',
      },
    },
  },
  tags: ['autodocs'],
    ...
    return (
      <div className="w-[800px] border rounded-xl overflow-hidden shadow-2xl">
        <Playback>
          ...
          <PlaybackControls>
            <PlaybackButton onClick={() => setCurrent(Math.max(0, current - 1))}>
              <ChevronLeft />
            </PlaybackButton>
            <PlaybackButton active>
              <Pause />
            </PlaybackButton>
            <PlaybackButton onClick={() => setCurrent(Math.min(45, current + 1))}>
              <ChevronRight />
            </PlaybackButton>
          </PlaybackControls>
          <PlaybackActions>
    ...
    <div className="p-20 bg-gradient-to-tr from-indigo-600 via-blue-700 to-cyan-800 rounded-[3rem] relative overflow-hidden">
      ...
      <div className="relative z-10 w-[700px]">
        <Playback {...args} className="border-white/10 rounded-t-2xl">
          ...
          <PlaybackControls>
            <PlaybackButton className="text-white hover:bg-white/10">
              <ChevronLeft />
            </PlaybackButton>
            <PlaybackButton active className="bg-white text-black hover:bg-white/90">
              <Play />
            </PlaybackButton>
            <PlaybackButton className="text-white hover:bg-white/10">
              <ChevronRight />

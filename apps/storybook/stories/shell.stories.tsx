...
import { Activity, Layout, CogFour as Settings, User, Search, Zap, Database, Shield } from '@aazucena/icons';
...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'The root application layout system. Manages primary navigation sidebars, persistent headers, and the main scrollable content area.',
      },
    },
  },
  tags: ['autodocs'],
    ...
    <Shell {...args}>
      <ShellSidebar className="w-72 bg-card border-r border-border p-8">
        ...
        <nav className="space-y-3">
          <Button variant="ghost" className="w-full justify-start h-12 rounded-xl gap-4 bg-accent/50">
        ...
        <div className="mt-auto pt-8 border-t">
          <div className="flex items-center gap-4 mb-8 px-2">
            <Avatar className="size-10"><AvatarFallback>AA</AvatarFallback></Avatar>
          ...
          <Button variant="outline" className="w-full h-12 rounded-full font-black uppercase tracking-widest text-[10px]">
      ...
      <ShellView>
        <ShellHeader className="justify-between px-10">
          <div className="flex items-center gap-4">
            <Badge variant="outline" animated>UPLINK_STABLE</Badge>
            <span className="text-[10px] font-black uppercase tracking-widest opacity-40">Primary_Cluster_US_East</span>
          </div>
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" className="rounded-full"><Search size={18}/></Button>
            <Button size="sm" className="rounded-full px-6">New_Deployment</Button>
          </div>
        </ShellHeader>
        <ShellMain>
          <ShellContent maxWidth="7xl">
            <div className="space-y-12">
              <div className="h-40 rounded-[2.5rem] bg-gradient-to-br from-primary/10 to-blue-500/10 border border-primary/20 flex flex-col justify-center px-12 gap-2">
                <h1 className="text-5xl font-black tracking-tighter uppercase">Intelligence_Center</h1>
                <p className="text-xs font-medium uppercase tracking-widest text-primary/60">Node Management & Telemetry Ingestion</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pb-20">
                {[1, 2, 3, 4, 5, 6].map(i => (
                  <div key={i} className="h-64 rounded-[2.5rem] bg-card border border-border/50 flex items-center justify-center opacity-20 border-dashed">
                    <span className="font-black uppercase tracking-widest text-xs">Module_Slot_0{i}</span>
                  </div>
                ))}
              </div>
            </div>
  ...
  render: (args) => (
    <Shell {...args}>
      <ShellSidebar className="w-20 bg-black border-r border-cyan-500/20 flex flex-col items-center py-10 gap-10">
        <IconBox variant="cyber" size="md"><Activity size={20} /></IconBox>
        <IconBox variant="default" size="md" className="opacity-20"><Shield size={20} /></IconBox>
        <IconBox variant="default" size="md" className="opacity-20"><Zap size={20} /></IconBox>
        <div className="mt-auto"><IconBox variant="default" size="md" className="opacity-20"><Settings size={20} /></IconBox></div>
      </ShellSidebar>
      <ShellView>
        <ShellHeader variant="cyber" className="justify-between px-8 border-cyan-500/10">
          <div className="flex items-center gap-4">
            <span className="font-mono text-cyan-500 italic uppercase text-[10px] tracking-widest">// AAZUCENA_LYTICS_v1.4</span>
            <div className="size-1.5 rounded-full bg-cyan-500 animate-pulse" />
          </div>
          <span className="font-mono text-[9px] text-cyan-500/40 uppercase">ENCLAVE_AUTH: VERIFIED</span>
        </ShellHeader>
        <ShellMain className="bg-zinc-950">
          <div className="p-12 font-mono text-cyan-500/60 text-[11px] leading-loose">
            <p className="text-cyan-400 font-bold uppercase mb-4 text-sm">{'>'} SYSTEM_INITIALIZATION_COMPLETE</p>

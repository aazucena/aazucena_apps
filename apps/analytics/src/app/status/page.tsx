    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-black text-zinc-900 dark:text-zinc-100 font-sans selection:bg-primary-500/30">
      <div className="max-w-3xl mx-auto px-6 py-20">
        ...
        <div className="flex flex-col items-center text-center mb-16">
          ...
          <h1 className="text-3xl font-black tracking-tighter uppercase mb-2">SYSTEM_INTEGRITY_LAB</h1>
          <p className="text-xs font-mono text-zinc-500 uppercase tracking-[0.3em] font-bold">Aldrin Azucena // Real-time Node Status</p>
        </div>

        {/* OVERALL STATUS CARD */}
        <div className={cn(
          "p-10 rounded-[3rem] border transition-all duration-700 mb-10",
          loading ? "bg-zinc-100 dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 animate-pulse" :
          isHealthy ? "bg-emerald-500/5 border-emerald-500/20 shadow-[0_0_50px_rgba(16,185,129,0.05)]" :
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-16">
          {loading ? (
            ...
            Object.entries(data?.services || {}).map(([name, svc]: [string, any]) => (
              <div key={name} className="p-6 bg-white dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-3xl flex items-center justify-between group hover:border-primary-500/30 transition-all">
                <div className="flex items-center gap-4">
                  <div className={cn(
                    "w-10 h-10 rounded-2xl flex items-center justify-center transition-colors",
                    svc.status === 'UP' ? "bg-emerald-500/10 text-emerald-500" : "bg-rose-500/10 text-rose-500"
                  )}>
                    {name.includes('db') ? <Activity size={20} /> :
                     name.includes('intel') ? <Terminal size={20} /> :
                     <Globe size={20} />}
                  </div>
                  <div>
                    <div className="text-[10px] font-black uppercase text-zinc-400 mb-0.5">{name}</div>
                    <div className="text-xs font-bold font-mono text-zinc-900 dark:text-zinc-100">{svc.status}</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-[8px] font-black uppercase text-zinc-500 mb-0.5 tracking-tighter">LATENCY</div>
                  <div className="text-[10px] font-mono font-bold text-zinc-600 dark:text-zinc-400">{svc.latency}ms</div>
                </div>
              </div>
            ))
          )}

...
import { Activity, Zap, Database, Globe, DangerTriangle, XCircle, ExternalLink } from '@mynaui/icons-react'; 
...
export default function PerformancePage() {
  ...
  const lcpValue = stats?.summary?.lcp_p75 ? `${(stats.summary.lcp_p75 / 1000).toFixed(2)}s` : '0.00s';
  ...
  const healthScore = totalMetrics > 0 
    ? `${((goodMetrics / totalMetrics) * 100).toFixed(0)}%`
    : '100%';
    ...
    <div className="space-y-10 pb-20">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
           ...
           <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Live_Stream_Active</span>
        </div>
      </div>

      {/* KPI METRICS */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6"> {/* Changed to 5 columns */}
        <MetricCard 
          label="LCP (P75)" 
          value={lcpValue} 
      ...
      <div className="bg-white/50 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] p-8 backdrop-blur-md shadow-sm dark:shadow-none transition-all duration-300">
        <div className="flex items-center justify-between mb-8 px-2">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-xs font-black text-zinc-900 dark:text-zinc-300 uppercase tracking-[0.2em]">Core Web Vitals Velocity</h3>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            {(stats?.errors?.top && stats.errors.top.length > 0) ? (
              stats.errors.top.map((error, idx) => (
                <div key={error.issue_id || idx} className="p-4 bg-zinc-50 dark:bg-zinc-950/50 border border-zinc-200 dark:border-zinc-800 rounded-2xl flex items-start gap-3">
                  <div className={cn(
                    "w-6 h-6 rounded-full flex items-center justify-center shrink-0",
                    error.level === 'fatal' ? "bg-rose-500 text-white" :
                    error.level === 'error' ? "bg-red-500 text-white" :
                    error.level === 'warning' ? "bg-amber-500 text-white" : "bg-zinc-400 text-white"
                  )}>
                    {error.level === 'fatal' || error.level === 'error' ? <XCircle size={14} /> : <DangerTriangle size={14} />} {/* Changed AlertTriangle to DangerTriangle */}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-mono font-bold text-zinc-900 dark:text-zinc-100 truncate">{error.message}</p>
                    <p className="text-[10px] text-zinc-500 dark:text-zinc-400 truncate">{error.culprit || 'unknown culprit'}</p>
                    <div className="flex items-center gap-2 mt-2">
                        <span className={cn(
                          "text-[9px] font-mono font-black uppercase tracking-widest px-2 py-0.5 rounded-full",
                          error.level === 'fatal' ? "bg-rose-500/10 text-rose-500" :
                          error.level === 'error' ? "bg-red-500/10 text-red-500" :
                          error.level === 'warning' ? "bg-amber-500/10 text-amber-500" : "bg-zinc-500/10 text-zinc-500"
                        )}>
                            {error.level}
                        </span>
                        <Link href={error.sentry_url} target="_blank" rel="noopener noreferrer" className="text-[9px] font-mono font-bold text-primary-500 hover:underline flex items-center gap-1">
        ...
        <div className="bg-white/50 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] p-8 backdrop-blur-md">
          <h3 className="text-xs font-black text-zinc-500 uppercase tracking-widest mb-8 px-2">Service Heartbeats</h3>
          <div className="grid grid-cols-1 gap-6"> {/* Adjusted to 1 column for better layout in half-width */}
            {(() => {
              // Deduplicate integrity logs by service name, keeping the latest one
              const uniqueServices = stats?.integrity?.reduce((acc: any, current: any) => {
              ...
              return serviceList.map((item: any, idx: number) => {
                ...
                const isCore = item.service.includes('cms') || item.service.includes('clickhouse') || item.service.includes('db');
                const Icon = isCore ? Database : (item.service.includes('api') ? Globe : Zap);
                
                return (
                  <div key={item.service || idx} className="group p-6 bg-zinc-50 dark:bg-zinc-950/50 border border-zinc-200 dark:border-zinc-800 rounded-[2rem] flex flex-col gap-4 transition-all hover:border-zinc-300 dark:hover:border-zinc-700 hover:shadow-lg dark:hover:shadow-primary-500/5">
                    <div className="flex items-center justify-between">
                      <div className={cn(
                        "w-10 h-10 rounded-xl flex items-center justify-center border transition-colors",
                        item.status === 'UP' ? "bg-emerald-500/5 border-emerald-500/20 text-emerald-500" : 
                        item.status === 'DEGRADED' ? "bg-amber-500/5 border-amber-500/20 text-amber-500" : 
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="w-2 h-2 rounded-full shadow-[0_0_8px]" /> {/* cn removed here */}
                      </div>
                    </div>
                    ...
                    <div className="pt-4 border-t border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
                      <div className="flex flex-col">
                        <span className="text-[8px] font-bold text-zinc-400 uppercase tracking-widest">Stability</span>
                        <span className="text-[10px] font-mono text-zinc-600 dark:text-zinc-400 font-bold">99.9% UPTIME</span>
                      </div>
                      <div className="text-right flex flex-col">
                        <span className="text-[8px] font-bold text-zinc-400 uppercase tracking-widest">Last Pulse</span>

  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      <div className="bg-white/50 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] p-8 backdrop-blur-md shadow-sm">
        <div className="flex items-center justify-between mb-8 px-2">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-xs font-black text-zinc-900 dark:text-zinc-300 uppercase tracking-[0.2em]">Revenue Velocity</h3>
          </div>
      ...
      <div className="bg-white/50 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] p-8 backdrop-blur-md">
        <h3 className="text-xs font-black text-zinc-500 uppercase tracking-widest mb-8 px-2">Recent Transactions</h3>
          ...
          <table className="w-full text-left font-mono text-[10px]">
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
              {stats?.history?.map((t, idx) => (
                <tr key={t.transaction_id || idx} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors group">
                  ...
                  <td className="py-4 px-4">
                    <span className={cn(
                        ...
                        t.provider === 'STRIPE' ? "bg-indigo-500/10 text-indigo-500" : "bg-sky-500/10 text-sky-500"
                    )}>
                        {t.provider}
                    </span>
                  </td>
              ...
              {(!stats?.history || stats.history.length === 0) && (
                <tr>
                  <td colSpan={5} className="py-20 text-center text-zinc-500 font-mono text-xs uppercase">No financial events detected yet.</td>
                </tr>
              )}
            </tbody>
          </table>

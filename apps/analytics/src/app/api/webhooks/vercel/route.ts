  ...
  try {
    ...
    const rows = logs
      ...
      .map(log => {
        ...
        const geo = proxy.geo || {};
        const uaString = Array.isArray(proxy.userAgent) ? proxy.userAgent[0] : (proxy.userAgent || '');
        ...
        return {
          timestamp: new Date(log.timestamp || Date.now()).toISOString().slice(0, 19).replace('T', ' '),
          id: log.id || '',
    ...
    if (error instanceof ZodError) {
        ...
        if ('errors' in zodError) {
             return NextResponse.json({ message: 'Invalid payload', errors: zodError.errors }, { status: 400 });
        }
        return NextResponse.json({ message: 'Invalid payload', errors: 'Unknown validation error' }, { status: 400 });
    }

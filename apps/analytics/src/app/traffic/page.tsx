  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      <div className="bg-white/50 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] p-8 backdrop-blur-md shadow-sm">
        <div className="flex items-center justify-between mb-8 px-2">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-xs font-black text-zinc-900 dark:text-zinc-300 uppercase tracking-[0.2em]">Traffic Velocity</h3>
          </div>
      ...
      <div className="bg-white/50 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] p-8 backdrop-blur-md">
        <h3 className="text-xs font-black text-zinc-500 uppercase tracking-widest mb-8 px-2">Global Distribution (Top 10)</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          {stats?.geo?.map((g, idx) => (
            <div key={g.country || idx} className="p-4 bg-zinc-50 dark:bg-zinc-950/50 border border-zinc-200 dark:border-zinc-800 rounded-2xl flex items-center justify-between">
              <span className="text-xs font-black uppercase tracking-widest text-zinc-400">{g.country || 'Unknown'}</span>
              <span className="text-sm font-mono font-bold text-zinc-900 dark:text-zinc-100">{g.visitors}</span>
            </div>
          ))}
          {(!stats?.geo || stats.geo.length === 0) && (
            <div className="col-span-full py-10 text-center text-zinc-500 font-mono text-xs uppercase">No geographic data ingested yet.</div>
          )}
        </div>
      </div>

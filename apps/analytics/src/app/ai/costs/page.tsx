...
export default function AiCostCenterPage() {
  ...
  const efficiencyScore = (totalSpendNum + totalSavingsNum) > 0 
    ...
    <div className="space-y-10 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-primary-500/10 border border-primary-500/20 rounded-full shrink-0 self-start">
           ...
           <span className="text-[10px] font-black text-primary-500 uppercase tracking-widest">Budget_Audit_Active</span>
        </div>
      </div>
      ...
      <div className="bg-white/50 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] p-8 backdrop-blur-md transition-all duration-300">
        <div className="flex items-center justify-between mb-8 px-2">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-xs font-black text-zinc-900 dark:text-zinc-300 uppercase tracking-[0.2em]">Spend Velocity Dynamics</h3>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
        ...
        <div className="bg-white/50 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] overflow-hidden backdrop-blur-md">
          <div className="px-8 py-6 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 flex items-center justify-between">
            <h3 className="text-xs font-black text-zinc-900 dark:text-zinc-300 uppercase tracking-[0.2em]">Model Efficiency Audit</h3>
            <span className="text-[9px] font-mono text-zinc-400 uppercase tracking-widest">7_Day_Analysis</span>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-zinc-200 dark:border-zinc-800">
                  <th className="px-6 py-4 text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest">Model_ID</th>
                  <th className="px-6 py-4 text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest text-right">Inferences</th>
                  <th className="px-6 py-4 text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest text-right">Actual_Spend</th>
                  <th className="px-6 py-4 text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest text-right">Fiscal_Savings</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/50">
                {(stats?.distribution || []).map((model: any) => (
                  <tr key={model.model} className="group hover:bg-zinc-100 dark:hover:bg-zinc-800/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-2 h-2 rounded-full bg-primary-500 group-hover:scale-125 transition-transform" />
                        <div>
                          <div className="font-bold text-zinc-900 dark:text-zinc-100 text-sm font-mono truncate max-w-[150px]">{model.model.split('/').pop()}</div>
                    ...
                    <td className="px-6 py-4 text-right">
                      <span className="font-mono text-xs font-bold text-zinc-700 dark:text-zinc-300">{model.inferences.toLocaleString()}</span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <span className="font-mono text-xs font-black text-primary-500">${model.spend.toFixed(4)}</span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex flex-col items-end">
                        <span className="font-mono text-xs font-black text-emerald-500">${(model.savings || 0).toFixed(4)}</span>
                        <div className="w-16 h-1 bg-zinc-200 dark:bg-zinc-800 rounded-full mt-1 overflow-hidden">
                          <div 
                            className="h-full bg-emerald-500" 
                            style={{ width: `${Math.min(((model.savings || 0) / (stats.summary.total_savings || 1)) * 100, 100)}%` }}
                          />
                        </div>
                      </div>
                    </td>
                ...
                {(!stats?.distribution || stats.distribution.length === 0) && (
                  <tr>
                    <td colSpan={3} className="py-20 text-center text-zinc-500 font-mono text-[10px] uppercase tracking-[0.3em] italic">
        ...
        <div className="bg-white/50 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] overflow-hidden backdrop-blur-md">
          <div className="px-8 py-6 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 flex items-center justify-between">
            <h3 className="text-xs font-black text-zinc-900 dark:text-zinc-300 uppercase tracking-[0.2em]">Agent Resource Allocation</h3>
            <span className="text-[9px] font-mono text-zinc-400 uppercase tracking-widest">7_Day_Analysis</span>
          </div>
          ...
          <div className="p-2 overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-zinc-200 dark:border-zinc-800">
                  <th className="px-6 py-4 text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest">Agent_Identity</th>
                  <th className="px-6 py-4 text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest text-right">Inferences</th>
                  <th className="px-6 py-4 text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest text-right">Total_Spend</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/50">
                {(stats?.agents || []).map((agent: any) => (
                  <tr key={agent.agent_name} className="group hover:bg-zinc-100 dark:hover:bg-zinc-800/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-2 h-2 rounded-full bg-secondary-500 group-hover:scale-125 transition-transform" />
                        <div>
                          <div className="font-bold text-zinc-900 dark:text-zinc-100 text-sm font-mono">{agent.agent_name}</div>
                          <div className="text-[10px] text-zinc-500 dark:text-zinc-500 uppercase tracking-wider font-mono opacity-60">
                            Neural_Node
                          </div>
                        </div>
                    ...
                    <td className="px-6 py-4 text-right">
                      <span className="font-mono text-xs font-bold text-zinc-700 dark:text-zinc-300">{agent.inferences.toLocaleString()}</span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex flex-col items-end">
                        <span className="font-mono text-xs font-black text-secondary-500">${agent.spend.toFixed(4)}</span>
                        <div className="w-16 h-1 bg-zinc-200 dark:bg-zinc-800 rounded-full mt-1 overflow-hidden">
                          <div 
                            className="h-full bg-secondary-500" 
                            style={{ width: `${Math.min((agent.spend / (stats.summary.total_spend || 1)) * 100, 100)}%` }}
                          />
                        </div>
                      </div>
                    </td>

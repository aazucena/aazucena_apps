 ...
 */
const JsonView = ({ data, keyName, level = 0 }: { data: any; keyName?: string; level?: number }) => {
  if (data === null) return <span className="text-rose-500/50 italic">null</span>;
  if (typeof data === 'boolean') {
    return (
      <span className={cn(
        "font-bold px-1.5 py-0.5 rounded text-[10px]",
        keyName === 'is_valid' && data === false ? "bg-rose-500 text-white animate-pulse" : 
        keyName === 'is_valid' && data === true ? "bg-emerald-500 text-white" :
        "text-amber-500"
      )}>
  ...
  if (typeof data === 'string') {
    ...
    const isMarkdownField = keyName?.toLowerCase().includes('reasoning') || 
    ...
    if (isMarkdownField) {
      return (
        <div className={cn(
          ...
          isInvalid ? "bg-rose-500/10 border-rose-500/50" : 
          isShade ? "bg-emerald-500/5 border-emerald-500/30" : "bg-primary-500/5 border-primary-500/30",
          ...
          <MarkdownRenderer 
            ...
            className={cn(
              ...
              isInvalid ? "text-rose-200" : isShade ? "text-emerald-700 dark:text-emerald-200" : "text-zinc-700 dark:text-zinc-200"
            )} 
          />
        </div>
      );
    }
    return <span className={cn("break-words leading-relaxed", isInvalid ? "text-rose-500 font-bold" : "text-zinc-600 dark:text-zinc-300")}>"{data}"</span>;
  }
      ...
      <div className="flex flex-col gap-1">
        {data.map((item, i) => (
          <div key={i} className="flex gap-2">
            <span className="text-zinc-400 dark:text-zinc-600 font-mono text-[10px] shrink-0">[{i}]</span>
  ...
  if (typeof data === 'object') {
    ...
    return (
      <div className={cn(
        ...
        hasFailedAudit && "bg-rose-500/5 p-4 rounded-2xl border border-rose-500/20 my-2 shadow-lg shadow-rose-500/5"
      )}>
        {hasFailedAudit && (
  ...
  const parsedObservation = useMemo(() => {
    ...
    try {
      ...
      if (jsonStart !== -1) {
        ...
        try { return { _info: prefix, ...JSON.parse(possibleJson) }; } catch { }
      }
      return obs;
    } catch { return obs; }
  }, [currentStep]);
    ...
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-primary-500/10 border border-primary-500/20 rounded-2xl shrink-0 self-start">
           ...
           <span className="text-[10px] font-black text-primary-500 uppercase tracking-widest">Neural_Sync_Active</span>
        </div>
      </div>
        ...
        <div className="xl:col-span-1 bg-white/50 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] flex flex-col overflow-hidden shadow-sm">
          <div className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50 flex items-center justify-between">
            ...
            <button 
              ...
              disabled={isLoading}
              className={cn("p-1.5 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-all text-zinc-400 hover:text-primary-500", isLoading && "animate-spin text-primary-500")}
              title="Refresh Data"
            >
              <Refresh size={14} />
            </button>
          ...
          <div className="flex-1 overflow-hidden relative p-4 flex flex-col">
            <div className="flex-1 overflow-y-auto space-y-2 scrollbar-thin scrollbar-thumb-zinc-300 dark:scrollbar-thumb-zinc-700 pr-2 -mr-1">
              {isLoading ? (
                <div className="py-20 text-center animate-pulse text-[10px] font-mono text-zinc-500 uppercase">Sequencing_Neural_Logs...</div>
              ) : trajectories.length === 0 ? (
                <div className="py-20 text-center text-[10px] font-mono text-zinc-500 uppercase italic">No_Trajectories_Detected</div>
              ) : (
                trajectories.map((t) => (
                  <button
                    key={t.id}
                    onClick={() => { setSelectedTrajectory(t); setCurrentStepIndex(0); }}
                    className={cn(
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[10px] font-mono font-bold opacity-70 truncate max-w-[100px]">{t.id}</span>
                      <span className="text-[10px] font-mono font-bold">R: {t.totalReward.toFixed(2)}</span>
                    </div>
                    <div className={cn("text-xs font-black uppercase tracking-wider truncate", selectedTrajectory?.id === t.id ? "text-white" : "text-zinc-900 dark:text-zinc-100")}>{t.agent}</div>
                    <div className="mt-2 flex items-center gap-2">
                      <div className={cn("px-1.5 py-0.5 rounded text-[8px] font-black", selectedTrajectory?.id === t.id ? "bg-white/20" : "bg-zinc-100 dark:bg-zinc-900 text-zinc-500")}>{t.length} STEPS</div>
                    </div>
                  </button>
                ))
              )}
        ...
        <div className="xl:col-span-3 flex flex-col gap-6 min-h-0">
          ...
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] flex flex-col overflow-hidden shadow-sm h-40 shrink-0">
            ...
            <div className="flex-1 relative overflow-hidden bg-zinc-50/30 dark:bg-zinc-900/10">
              <NeuralMap steps={selectedTrajectory?.steps || []} currentStepIndex={currentStepIndex} />
            </div>
          </div>
            ...
            <div className="px-8 py-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-6">
                <div className="text-[10px] font-mono text-primary-500 font-black uppercase tracking-[0.2em]">
                  Step_{ (currentStepIndex + 1).toString().padStart(3, '0') } / { selectedTrajectory?.length || 0 }
                </div>
                <div className="h-4 w-px bg-zinc-200 dark:bg-zinc-800" />
                <div className="flex items-center gap-2">
                  <button onClick={() => setCurrentStepIndex(Math.max(0, currentStepIndex - 1))} className="p-2 hover:bg-zinc-200 dark:hover:bg-zinc-800 rounded-xl transition-all"><ChevronLeft size={18} /></button>
                  <button onClick={() => setIsPlaying(!isPlaying)} className="w-10 h-10 rounded-xl bg-primary-500 text-white flex items-center justify-center hover:bg-primary-600 transition-all shadow-lg shadow-primary-500/20">{isPlaying ? <Pause size={20} /> : <Play size={20} />}</button>
                  <button onClick={() => setCurrentStepIndex(Math.min((selectedTrajectory?.length || 1) - 1, currentStepIndex + 1))} className="p-2 hover:bg-zinc-200 dark:hover:bg-zinc-800 rounded-xl transition-all"><ChevronRight size={18} /></button>
                </div>
              </div>

              <div className="flex items-center gap-8">
                <div className="text-right">
                  <div className="text-[8px] font-black text-zinc-400 uppercase tracking-widest leading-none mb-1">Outcome_Reward</div>
                  <div className={cn("text-lg font-black font-mono leading-none", (currentStep?.reward ?? 0) >= 0 ? "text-emerald-500" : "text-rose-500")}>{(currentStep?.reward ?? 0) >= 0 ? '+' : ''}{currentStep?.reward ?? 0}</div>
                </div>
                <div className="text-right">
                  <div className="text-[8px] font-black text-zinc-400 uppercase tracking-widest leading-none mb-1">Decision_Confidence</div>
                  <div className="text-lg font-black font-mono leading-none text-zinc-900 dark:text-zinc-100">{((currentStep?.probability || 0) * 100).toFixed(1)}%</div>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 h-full">
                ...
                <div className="lg:col-span-4 space-y-6 overflow-y-auto scrollbar-none pr-2">
                  <div className="space-y-2">
                    <div className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">Cognitive_Decision</div>
                  ...
                  <div className="space-y-2">
                    <div className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">Step_Metadata</div>
                    <div className="bg-zinc-900 rounded-2xl p-4 border border-zinc-800 relative group/meta h-48 overflow-y-auto scrollbar-none">
                      <button onClick={() => navigator.clipboard.writeText(JSON.stringify(currentStep?.metadata || {}, null, 2))} className="absolute top-3 right-3 opacity-0 group-hover/meta:opacity-100 p-1.5 bg-primary-500 text-white rounded-lg transition-all shadow-lg"><Copy size={12}/></button>
                      <pre className="text-[10px] font-mono text-emerald-500/80 leading-relaxed">{JSON.stringify(currentStep?.metadata, null, 2)}</pre>
                    </div>
                  </div>
                </div>
                ...
                <div className="lg:col-span-8 flex flex-col h-full min-h-0 space-y-2">
                  <div className="flex items-center justify-between shrink-0">
                    <div className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">Agent_Sensory_Input (Observation)</div>
                    <button onClick={() => navigator.clipboard.writeText(currentStep?.observation || '')} className="flex items-center gap-1.5 px-3 py-1 bg-primary-500/10 border border-primary-500/20 text-primary-500 rounded-xl text-[9px] font-black hover:bg-primary-500 hover:text-white transition-all"><Copy size={12}/> Copy Raw</button>
                  </div>
                  <div className="flex-1 bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-[2rem] p-6 shadow-inner relative overflow-hidden flex flex-col">
                    <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-300 dark:scrollbar-thumb-zinc-700 pr-4 -mr-2">
                      <div className="text-[12px] leading-relaxed">
                        {parsedObservation ? <JsonView data={parsedObservation} /> : <span className="text-zinc-500 italic uppercase">Awaiting_Signal...</span>}
                      </div>
                    </div>
                  </div>
                </div>

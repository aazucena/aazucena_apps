...
export default function AiTerminalPage() {
  ...
  const allMessagesMap = useSelector((state: RootState) => state.chat.conversations[activeConversationId || '']?.messages || {});
  ...
  const { status, sendMessage, messages: streamMessages } = useChat({
        ...
        for (const eventStr of events) {
          ...
          if (data) {
            if (eventType === 'final_response') {
              ...
              setThoughtTrace(prev => prev[prev.length - 1] === `Action: ${data}` ? prev : [...prev, `Action: ${data}`]);
            } else if (eventType === 'status') {
              setThoughtTrace(prev => prev[prev.length - 1] === `System: ${data}` ? prev : [...prev, `System: ${data}`]);
            }
          }
        }
        return remaining;
    ...
    <div className="h-[calc(100vh-8rem)] flex gap-6">
      ...
      <div className={cn(
        ...
        <div className="p-6 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between bg-zinc-50/50 dark:bg-zinc-900/30">
          ...
          <button onClick={() => dispatch(createNewChat())} className="p-2 bg-primary-500 text-white rounded-xl hover:bg-primary-600 transition-all shadow-lg shadow-primary-500/20 active:scale-95" title="New Chat"><Plus size={16} /></button>
        </div>
            ...
            <div key={conv.id} className="relative group">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[8px] font-mono opacity-50 mt-1 uppercase">{new Date(conv.updatedAt).toLocaleDateString()}</p>
                </div>
              </button>
        ...
        <div className="p-4 border-t border-zinc-200 dark:border-zinc-800">
          <button onClick={() => dispatch(clearAllHistory())} className="w-full py-2 rounded-xl text-[10px] font-black uppercase tracking-widest text-rose-500 hover:bg-rose-500/10 transition-colors flex items-center justify-center gap-2"><Trash size={14} /> Clear All</button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0">
          <div className="flex items-center gap-4">
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className={cn("p-3 rounded-2xl border transition-all", isSidebarOpen ? "bg-primary-500 text-white border-primary-600" : "bg-white dark:bg-zinc-900 text-zinc-500 border-zinc-200 dark:border-zinc-800")}><Activity size={20} /></button>
            <div>
          ...
          <div className="flex items-center gap-2 bg-zinc-100 dark:bg-zinc-900/50 p-1 rounded-xl border border-zinc-200 dark:border-zinc-800">
            {AVAILABLE_MODELS.map((model) => (
              <button key={model.id} onClick={() => setSelectedModel(model.id)} className={cn("px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all", selectedModel === model.id ? "bg-white dark:bg-zinc-800 text-primary-500 shadow-sm border border-zinc-200 dark:border-zinc-700" : "text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300")}>{model.name.split(' ')[0]}</button>
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] flex flex-col overflow-hidden shadow-2xl shadow-primary-500/5">
          <div className="px-8 py-5 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between bg-zinc-50 dark:bg-zinc-900/50">
            <div className="flex items-center gap-6 flex-1 min-w-0 group/title relative">
              ...
              <div className="relative flex-1 min-w-0">
                ...
                <div className="absolute top-full left-0 mt-3 opacity-0 group-hover/title:opacity-100 pointer-events-none transition-all duration-200 z-50 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-4 rounded-2xl shadow-2xl min-w-[300px] max-w-[600px] border-t-4 border-t-primary-500">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-primary-500 text-[8px] font-mono font-black uppercase tracking-tighter">THREAD_CONCEPTION</span>
              ...
              <div className="flex items-center gap-2 shrink-0 text-[10px] font-mono text-zinc-400 font-bold uppercase tracking-widest">
                <TerminalIcon size={12} className={cn(isLoading ? "text-primary-500 animate-spin" : "text-zinc-400")} /> 
                status: <span className={cn(isLoading ? "text-primary-500" : "text-zinc-500")}>{isLoading ? 'INFERENCE_ACTIVE' : 'IDLE'}</span>
              </div>
            </div>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-10 scrollbar-thin scrollbar-thumb-zinc-200 dark:scrollbar-thumb-zinc-800">
            {displayMessages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-50">
                <div className="w-16 h-16 rounded-3xl bg-zinc-100 dark:bg-zinc-900 flex items-center justify-center text-zinc-400"><Activity size={32} /></div>
                <p className="text-xs font-black uppercase tracking-widest text-zinc-900 dark:text-zinc-100">Core Ready for Analysis</p>
              </div>
            )}

            {displayMessages.map((m) => {
              const siblings = isLocal ? Object.values(activeConv?.messages || {}).filter(s => s.parentId === m.parentId) : [];
              const currentVersionIdx = isLocal ? siblings.findIndex(s => s.id === m.id) : 0;

              return (
                <div key={m.id} className={cn("flex gap-4 group/msg", m.role === 'user' ? "flex-row-reverse" : "flex-row")}>
                  <div className={cn("w-8 h-8 rounded-xl flex items-center justify-center shrink-0 border transition-transform group-hover/msg:scale-110",
                    m.role === 'user' ? "bg-zinc-100 dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 text-zinc-500" : "bg-primary-500/10 border-primary-500/20 text-primary-500 shadow-[0_0_15px_rgba(16,185,129,0.1)]")}>
                    {m.role === 'user' ? <Activity size={16} /> : <Sparkles size={16} />}
                  </div>

                  <div className={cn("flex flex-col max-w-[85%] relative group/bubble", m.role === 'user' ? "items-end text-right" : "items-start text-left")}>
                    <div className="flex items-center gap-2 mb-1.5 px-1">
                      <span className="text-[10px] font-black uppercase tracking-widest text-zinc-400">{m.role === 'user' ? 'Operator' : 'AI_Core'}</span>
                      {isLocal && siblings.length > 1 && (
                        <div className="flex items-center gap-1 bg-zinc-100 dark:bg-zinc-900 px-1.5 py-0.5 rounded-full border border-zinc-200 dark:border-zinc-800 scale-90">
                          <button onClick={() => switchVersion(m.parentId, -1)} className="hover:text-primary-500 transition-colors"><ChevronLeft size={10}/></button>
                          <span className="text-[8px] font-mono font-black">{currentVersionIdx + 1} / {siblings.length}</span>
                          <button onClick={() => switchVersion(m.parentId, 1)} className="hover:text-primary-500 transition-colors"><ChevronRight size={10}/></button>
                        </div>
                      )}
                    </div>

                    <div className={cn("p-5 rounded-2xl text-sm font-mono leading-relaxed transition-all border group relative shadow-sm",
                      m.role === 'user' ? "bg-zinc-100 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 rounded-tr-none border-zinc-200 dark:border-zinc-800" : "bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 text-zinc-800 dark:text-zinc-200 rounded-tl-none")}>
                      {editingId === m.id ? (
                        <div className="space-y-3">
                          <textarea value={editingValue} onChange={(e) => setEditingValue(e.target.value)} className="w-full bg-transparent border-none focus:ring-0 p-0 text-sm font-mono resize-none min-h-[60px]" autoFocus />
                          <div className="flex items-center gap-2 pt-2 border-t border-zinc-200/50 dark:border-zinc-800/50">
                            <button onClick={() => handleSaveEdit(m)} disabled={isLoading || !editingValue.trim() || editingValue === (m.parts[0]?.text || '')} className="px-3 py-1 bg-primary-500 text-white rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-primary-600 transition-colors disabled:opacity-50">Branch & Resend</button>
                            <button onClick={() => setEditingId(null)} disabled={isLoading} className="px-3 py-1 bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 rounded-lg text-[10px] font-black uppercase tracking-widest">Cancel</button>
                          </div>
                        </div>
                      ) : (
                        <>
                          {m.parts.map((part, i) => {
                            ...
                            if (part.type === 'reasoning') return (
                              <div key={i} className="text-[11px] text-zinc-500 italic border-l-2 border-primary-500/30 pl-3 my-3 font-mono bg-primary-500/5 py-2 rounded-r-lg">
                                <span className="text-[9px] font-black text-primary-500/50 block mb-1 uppercase tracking-tighter">INTERNAL_REASONING</span>
                                {part.text}
                              </div>
                            );
                            return null;
                          ...
                          <div className="mt-4 pt-3 border-t border-zinc-200/50 dark:border-zinc-800/50 flex items-center gap-2 opacity-30 group-hover/msg:opacity-100 transition-all duration-300">
                            <button onClick={() => handleCopy(m.id, m.parts.map(p => p.text).join('\n'))} disabled={isLoading} className={cn("flex items-center gap-1 px-2 py-1 rounded-md transition-all text-[9px] font-black uppercase tracking-tighter border", copiedId === m.id ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : "hover:bg-primary-500/10 hover:text-primary-500 text-zinc-500 border-transparent hover:border-primary-500/20")}>
                              {copiedId === m.id ? <><Check size={12} /> COPIED</> : <><Copy size={12} /> COPY</>}
                            </button>
                            {m.role === 'user' && <button onClick={() => handleEdit(m)} disabled={isLoading} className="flex items-center gap-1 px-2 py-1 hover:bg-amber-500/10 hover:text-amber-500 text-zinc-500 rounded-md transition-all text-[9px] font-black uppercase tracking-tighter border border-transparent hover:border-amber-500/20"><Edit size={12} /> EDIT</button>}
                            {m.role === 'assistant' && <button onClick={() => handleRegenerate(m)} disabled={isLoading} className="flex items-center gap-1 px-2 py-1 hover:bg-emerald-500/10 hover:text-emerald-500 text-zinc-500 rounded-md transition-all text-[9px] font-black uppercase tracking-tighter border border-transparent hover:border-emerald-500/20"><Refresh size={12} /> REGEN</button>}
                          </div>
                        </>
                      )}
                    </div>
            ...
            {isLocalLoading && (
              <div className="flex gap-4">
                <div className="w-8 h-8 rounded-xl bg-primary-500/10 border border-primary-500/20 flex items-center justify-center text-primary-500 animate-pulse"><Sparkles size={16} /></div>
                  ...
                  <div className="space-y-2 max-w-[400px] bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 text-zinc-800 dark:text-zinc-200 shadow-sm p-5 rounded-2xl text-sm font-mono leading-relaxed transition-all border animate-pulse">
                    {thoughtTrace.length > 0 ? (
                      thoughtTrace.map((log, i) => (
                        <div key={i} className="flex items-center gap-2 text-[10px] font-mono text-zinc-400 animate-in fade-in slide-in-from-left-2 duration-300">
          ...
          <div className="p-6 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/30">
            <form onSubmit={onSend} className="relative flex items-end gap-3">
              <div className="relative flex-1">
                ...
                />
                <div className="absolute right-3 bottom-3"><button type="submit" disabled={isLoading || !input.trim() || !activeConversationId} className="w-10 h-10 rounded-xl bg-primary-500 text-white flex items-center justify-center hover:bg-primary-600 transition-all disabled:opacity-50 shadow-lg shadow-primary-500/20"><Send size={18} /></button></div>
              </div>
            </form>
            <div className="mt-3 flex items-center justify-between px-2 text-[8px] font-mono text-zinc-400 uppercase">
              <div className="flex items-center gap-4">
                <span className="flex items-center gap-1"><div className="w-1 h-1 rounded-full bg-emerald-500" /> Mode: Intel_Core_V2</span>
                <span className="flex items-center gap-1 text-zinc-500">History_Nodes: {Object.keys(allMessagesMap).length}</span>
              </div>
              <div className="flex items-center gap-3"><span>Shift+Enter for newline</span><span className="text-primary-500 font-black">Enter to infer</span></div>
            </div>
          </div>
        </div>
      </div>

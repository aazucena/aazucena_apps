  ...
  return (
    <div className="bg-white/50 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-[2rem] overflow-hidden backdrop-blur-md shadow-lg transition-all duration-500">
      ...
      <div className={cn(
        ...
        status === 'CRITICAL' ? "bg-rose-500/10 border-rose-500/20" :
        ...
        <div className="flex items-center gap-4">
          <div className={cn(
            ...
            status === 'CRITICAL' ? "bg-rose-500 text-white shadow-[0_0_20px_rgba(244,63,94,0.4)] animate-pulse" :
            status === 'WARNING' ? "bg-amber-500 text-white shadow-[0_0_20px_rgba(245,158,11,0.4)]" :
            "bg-emerald-500 text-white shadow-[0_0_20px_rgba(16,185,129,0.2)]"
          )}>
            {status === 'CRITICAL' ? <XCircle size={24} /> :
             status === 'WARNING' ? <DangerTriangle size={24} /> :
             <Shield size={24} />}
          </div>
          <div>
        ...
        <div className="hidden md:flex items-center gap-6">
          <div className="flex flex-col items-end">
            <span className="text-[8px] font-black text-zinc-400 uppercase tracking-widest">Active_Alerts</span>
            <span className="text-sm font-mono font-bold text-zinc-900 dark:text-zinc-100">{alerts.length}</span>
          </div>
          <div className="flex flex-col items-end">
            <span className="text-[8px] font-black text-zinc-400 uppercase tracking-widest">Neural_State</span>
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={cn(
                      ...
                      alert.level === 'CRITICAL' ? "bg-rose-500 shadow-[0_0_8px_#f43f5e]" : "bg-amber-500 shadow-[0_0_8px_#f59e0b]"
                    )} />
                    <div>
                      ...
                      <div className="text-[9px] text-zinc-500 uppercase mt-0.5">
                        Violation: <span className="font-bold text-zinc-700 dark:text-zinc-300">{alert.value}</span> / Limit: {alert.threshold}
                      </div>
                    </div>
                  </div>

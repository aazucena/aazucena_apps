...
import type { StreamGraphStep } from '../transformers';
...
export function StreamGraph({ data }: StreamGraphProps) {
  ...
  useEffect(() => {
    if (!svgRef.current || !filteredData.length || dimensions.width === 0 || !colorScale) return;
  ...
  return (
    <div ref={containerRef} className="w-full relative rounded-xl p-4 border border-gray-100 dark:border-gray-800">
      ...
      <div className="mt-4 flex flex-wrap justify-center gap-4">
        {colorScale && keys.map(key => (
          <div key={key} className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full" style={{ backgroundColor: colorScale(key) as string }}></div>

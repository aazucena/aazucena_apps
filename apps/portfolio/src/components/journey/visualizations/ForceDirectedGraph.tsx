...
import { skillSearchQueryStore } from '../store';
import * as d3 from 'd3';
import type { SkillsNetworkData, SkillNode, SkillLink } from '../transformers';
...
export function ForceDirectedGraph({ data, onNodeClick }: ForceDirectedGraphProps) {
    ...
    const handleResize = () => {
      if (containerRef.current) {
        setDimensions({
          ...
          height: Math.max(Math.min(containerRef.current.clientWidth * 0.8, 700), 500),
        });
      }
    };
  ...
  useEffect(() => {
    ...
    const uniqueCategories = Array.from(new Set(data.nodes.map((n) => n.category)));
    ...
    const simulation = d3
      ...
      .force('collision', d3.forceCollide().radius((d: any) => collisionRadius + Math.sqrt(d.size) * 4));
    ...
    node
      ...
      .on('mouseenter', function (event, d) {
        d3.select(this).transition().attr('r', (d) => 9 + Math.sqrt((d as { size: number }).size) * 4);
      ...
      .on('mouseleave', function (event, d) {
        d3.select(this).transition().attr('r', (d) => 6 + Math.sqrt((d as { size: number }).size) * 4);
        ...
        if (query) {
          node.style('opacity', (n) => n.name.toLowerCase().includes(query) ? 1 : 0.1);
        } else {
    ...
    <div ref={containerRef} className="w-full relative">
      ...
      <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
        <button
          ...
          className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm p-2 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all group w-fit"
          title="Force Simulation Settings"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`${showControls ? 'rotate-90' : ''} transition-transform`}><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>
        </button>

        {showControls && (
          <div className="bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm p-4 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 w-64 space-y-4 animate-in fade-in slide-in-from-left-2">
            <div>
              <div className="flex justify-between items-center mb-1">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest">Repulsion</label>
                <span className="text-xs font-mono text-blue-600">{chargeStrength}</span>
              </div>
              <input
                type="range"
                min="-1000"
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest">Distance</label>
                <span className="text-xs font-mono text-blue-600">{linkDistance}</span>
              </div>
              <input
                type="range"
                min="50"
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest">Spacing</label>
                <span className="text-xs font-mono text-blue-600">{collisionRadius}</span>
              </div>
              <input
                type="range"
                min="5"
            ...
            <button
              onClick={handleReset}
              className="w-full py-2 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-lg text-xs font-bold hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
            >
              Reset to Defaults
            </button>
          </div>
      ...
      <svg
        ...
        className="w-full bg-gray-50 dark:bg-gray-900 rounded-3xl"
      />
      <div className="mt-4 flex flex-wrap gap-4 justify-center text-sm">
        {Array.from(new Set(data.nodes.map((n) => n.category))).map((category) => (
                ...
                backgroundColor:
                  category === 'Other'
                    ...
                    : [
                        ...
                        Array.from(new Set(data.nodes.map((n) => n.category))).indexOf(
                          category
                        ) % 9
                      ],
              }}

...
export function SpiderChart({ data, selectedYears }: SpiderChartProps) {
  ...
  const categories = useMemo(() => {
    ...
    return Object.keys(data[0]!.categories).filter(
      (cat) => cat !== "Total Skills",
    );
  }, [data]);
  ...
  useEffect(() => {
    ...
    if (selectedYears && selectedYears.length > 0) {
      ...
      yearsToCompare = allYears.filter(
        (_, index, arr) =>
    ...
    <div ref={containerRef} className="w-full">
      <div className="relative">
        <div className="absolute top-0 right-0 z-10 p-4">
          <ExportControls
            svgRef={svgRef}
            fileName={`skill-growth-${currentYear}`}
          />
        </div>
        <svg
          ref={svgRef}
          ...
          className="w-full overflow-visible rounded-3xl bg-gray-50 p-4 dark:bg-gray-900"
        />
      </div>

      {/* Controls */}
      <div className="my-8 flex flex-col items-center gap-4 px-4 sm:flex-row">
        ...
        <div
          className={`flex w-full flex-1 items-center gap-3 transition-opacity duration-300 sm:w-auto ${showComparison ? "pointer-events-none opacity-30" : "opacity-100"}`}
        >
          ...
          />
          <span className="w-10 text-[10px] font-bold text-gray-400">

...
import type { SankeyData } from "~/lib/transformers/journey";
...
export function SankeyDiagram({ data }: SankeyDiagramProps) {
    ...
    const visibleNodes = data.nodes.filter((n) => {
      ...
      if (n.id.startsWith("skill_") || n.id.startsWith("group_")) {
        ...
        const incomingLink = data.links.find(
          (l) => l.target === n.id && l.source.startsWith("cat_"),
        );
        if (incomingLink) {
          const catNode = data.nodes.find(
            (node) => node.id === incomingLink.source,
          );
  ...
  useEffect(() => {
    if (!svgRef.current || !filteredData.nodes.length || dimensions.width === 0)
      return;
    ...
    const getNodeColor = (d: any) => {
      ...
      if (d.id.startsWith("skill_") || d.id.startsWith("group_")) {
        ...
        if (incomingLink)
          return categoryColor((incomingLink.source as any).name);
        return "#94a3b8";
      }
    ...
    node
      ...
      .attr("font-weight", (d: any) =>
        d.id.startsWith("cat_") ||
        d.id.startsWith("exp_") ||
        d.id.startsWith("prj_") ||
        d.id.startsWith("edu_")
          ? "700"
          : "400",
      )
  ...
  return (
    <div ref={containerRef} className="relative w-full overflow-hidden p-4">
      ...
      <svg
        ...
        className="w-full rounded-3xl bg-gray-50 p-4 text-gray-900 dark:bg-gray-900 dark:text-gray-100"
      />
      <div className="mt-4 flex justify-center gap-6 text-xs font-bold tracking-widest text-gray-500 uppercase">

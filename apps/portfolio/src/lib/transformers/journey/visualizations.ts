...
export function transformToSankeyData(
  ...
  const processEntity = (
    ...
    skills.forEach((s) => {
      ...
      categorySkillLinks.set(
        catLinkKey,
        (categorySkillLinks.get(catLinkKey) || 0) + 1,
      );
      const entLinkKey = `${skillNodeId}|${entityId}`;
      skillEntityLinks.set(
        entLinkKey,
        (skillEntityLinks.get(entLinkKey) || 0) + 1,
      );
    });
  };
  ...
  projects
    .slice(0, 6)
    .forEach((proj) => processEntity(proj.title, proj.techStack || [], "prj"));
  education
    .slice(0, 2)
    .forEach((edu) => processEntity(edu.institution, edu.skills || [], "edu"));
  ...
  while (current <= now) {
    ...
    experiences.forEach((exp) =>
      checkActive(
        ...
        exp.isCurrent ? now : new Date(exp.endDate || Date.now()),
        exp.skills,
      ),
    );
    education.forEach((edu) =>
      checkActive(
        ...
        edu.current ? now : new Date(edu.graduationDate || Date.now()),
        edu.skills || [],
      ),
    );
    projects.forEach((proj) =>
      checkActive(
        proj.startDate || proj.createdAt,
        proj.endDate || proj.updatedAt || proj.createdAt,
        proj.techStack || [],
      ),
    );

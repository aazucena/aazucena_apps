...
export type SkillsOverTime = SpiderChartData;
...
export function transformToSkillsEvolution(
  ...
  const addSkillsToYear = (year: number, skills: any[]) => {
  ...
  return years.map((year) => {
    ...
    const axes: { axis: string; value: number }[] = [
      { axis: "Total Skills", value: totalSkills },
    ];
    yearSkillsMap.forEach((skills, categoryName) => {
      axes.push({ axis: categoryName, value: skills.size });
    });

    return { name: String(year), axes };
  });
}
...
export function transformToSkillsNetwork(
  ...
  const findCoOccurringSkills = (skills: any[]) => {

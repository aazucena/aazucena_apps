 ...
 */
export function transformSkillCategory(
  strapiCategory: StrapiSkillCategory
): SkillCategoryInfo {
  return {
    ...
    icon: strapiCategory.icon ? getIconComponent(strapiCategory.icon) : undefined,
...
export function transformSkillCategories(
  ...
  const visible = strapiCategories.filter(
    (category) => category.display !== 'hidden'
  );

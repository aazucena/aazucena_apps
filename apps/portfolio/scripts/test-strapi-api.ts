  ...
  try {
    ...
    if (data.data) {
      ...
      if (recordCount === 0 && !options?.expectEmpty) {
        logWarning(`${name} - No data found (this might be expected if you haven't populated content yet)`);
      } else {
    ...
    logError(`${name} - ${error instanceof Error ? error.message : 'Unknown error'}`);
    return false;
  }
}
...
async function runTests() {
  ...
  const collectionTypes = [
    ...
    { name: 'Form Submissions', endpoint: 'form-submissions', expectEmpty: true },
    { name: 'Easter Egg Completions', endpoint: 'easter-egg-completions', expectEmpty: true },
  ];
  ...
  const totalTests = singleTypeResults.length + collectionTypeResults.length + 1;
  const passedTests = [...singleTypeResults, ...collectionTypeResults, populatedResult].filter(
    (r) => r
  ).length;
  ...
  if (passedTests === totalTests) {
    ...
    logInfo('3. Missing permissions → Verify token permissions in Strapi admin');
  }
